...
- [ ] `researchService.ts` rewritten with `executeHybridResearch`
- [ ] Parallel execution of Semantic Scholar + Gemini Grounding
- [ ] Results deduplicated by URL
- [ ] Records saved as metadata-only (no `fileData`)
- [ ] `smartDownload` import removed from this file
- [ ] `execute/route.ts` updated
