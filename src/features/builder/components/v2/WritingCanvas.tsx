...
export function WritingCanvas({ title, content, onValidChange, headerRight, saveStatus: externalSaveStatus, onEnhanceClick, onEditorReady, projectId }: WritingCanvasProps) {
    ...
    return (
        <div className="flex-1 flex flex-col relative bg-paper h-full overflow-hidden">
            {/* Toolbar */}
            <div className="flex items-center justify-between px-3 md:px-6 py-3 border-b border-rule bg-writing shrink-0 z-10 backdrop-blur-sm">
                <div className="flex items-center gap-1 min-w-0 overflow-x-auto">
                    {title && <h2 className="text-sm font-bold text-ink mr-2">{title}</h2>}
                    <div className="h-4 w-px bg-rule mx-2" />
                    <button onClick={() => toggleFormatting('bold')} className="min-w-11 min-h-11 flex items-center justify-center text-ink-muted hover:text-rust hover:bg-selection rounded-md transition-all" title="Bold">
                        <Bold className="w-4 h-4" />
                    </button>
                    <button onClick={() => toggleFormatting('italic')} className="min-w-11 min-h-11 flex items-center justify-center text-ink-muted hover:text-rust hover:bg-selection rounded-md transition-all" title="Italic">
                        <Italic className="w-4 h-4" />
                    </button>
                    <button onClick={() => toggleFormatting('heading')} className="min-w-11 min-h-11 flex items-center justify-center text-ink-muted hover:text-rust hover:bg-selection rounded-md transition-all" title="Heading">
                        <Heading className="w-4 h-4" />
                    </button>
                    <button onClick={() => toggleFormatting('list')} className="min-w-11 min-h-11 flex items-center justify-center text-ink-muted hover:text-rust hover:bg-selection rounded-md transition-all" title="List">
                        <List className="w-4 h-4" />
                    </button>
                    <button onClick={() => toggleFormatting('image')} className="min-w-11 min-h-11 flex items-center justify-center text-ink-muted hover:text-rust hover:bg-selection rounded-md transition-all" title="Image">
                        <ImageIcon className="w-4 h-4" />
                    </button>
                    <button onClick={() => toggleFormatting('table')} className="min-w-11 min-h-11 flex items-center justify-center text-ink-muted hover:text-rust hover:bg-selection rounded-md transition-all" title="Table">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar px-4 py-6 md:px-8 lg:px-12">
                <div className="max-w-[75ch] mx-auto h-full min-h-[500px] bg-writing border border-rule border-l-[5px] border-l-rust px-4 md:px-8 py-8">
                    <NovelEditor
                        content={content || ''}
            ...
            <div className="absolute bottom-4 right-4 z-20">
                <button
                    ...
                    className="group flex items-center gap-2 px-4 py-3 min-h-11 bg-rust hover:bg-rust/90 text-writing rounded-md transition-colors"
                >
