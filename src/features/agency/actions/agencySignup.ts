        ...
        if (!user) {
            try {
                ...
                if (newUser?.user) {
                    ...
                    // CRITICAL FIX: If we auto-generated the password, user needs a way to login.
                    // Instead of just nullifying, we should initiate a magic link flow or trust the forgot-password flow.
                    // For now, since this is a server action, sending a real-time email might be slow.
                    // We will set their account to require a reset and optionally trigger an email if possible in this scope.
                    ...
                    if (isAutoGenerated) {
                        ...
                        // We also trigger a magic link email so they can login immediately
                        try {
                            await auth.api.signInMagicLink({
                                body: { email: validated.email, callbackURL: '/dashboard' },
                                headers: await headers()
                            });
                        } catch (emailErr) {
                            console.error("Failed to auto-send magic link:", emailErr);
                            // Not blocking, user can use "Forgot Password" manually
                        }
                    }
                }
            } catch (authError) {
